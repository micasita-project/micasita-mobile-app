    ...
    <View style={styles.container}>
        ...
        <View style={styles.transportChipsRow}>
          {TRANSPORT_MODES.map((mode) => {
            ...
            const timeMin = newHomeRoutes
              ? Math.round(newHomeRoutes[mode.id].timeMinutes)
              : null;
            return (
              <TouchableOpacity
                key={mode.id}
              ...
              >
                ...
                <Text
                  style={[
                    ...
                    isActive && { color: TRANSPORT_MODE_COLORS[mode.id] },
                  ]}
                >
                  {mode.label}
                </Text>
                {isActive && timeMin !== null && (
                  <Text
                    style={[
                      ...
                      { color: TRANSPORT_MODE_COLORS[mode.id] },
                    ]}
                  >
                    · {timeMin}min
                  </Text>
      ...
      <View style={styles.mapContainer}>
        ...
        >
          ...
          {!isGuest &&
            activeWorkplace?.work_lat &&
            activeWorkplace?.work_lon && (
              <Marker
                coordinate={{
        ...
        {selectedHousing &&
          (() => {
            ...
            const modeIcon = TRANSPORT_MODES.find(
              (m) => m.id === selectedMode,
            )?.icon;
            ...
            return (
              ...
              >
                ...
                >
                  {selectedHousing.images &&
                  selectedHousing.images.length > 0 ? (
                    <Image
                  ...
                  <View style={styles.cardRight}>
                    <Text style={styles.cardPrice}>
                      {formatPrice(
                        selectedHousing.price,
                        selectedHousing.currency,
                      )}{" "}
                      <Text style={styles.cardPriceUnit}>/mes</Text>
                    </Text>
                    <Text style={styles.cardSpecs} numberOfLines={1}>
                      {selectedHousing.district} · {selectedHousing.bedrooms}{" "}
                      hab · {selectedHousing.total_area_sqm} m²
                    </Text>
                    <View
                      style={{
                        flexDirection: "row",
